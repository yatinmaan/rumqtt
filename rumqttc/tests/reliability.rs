...
async fn connection_should_timeout_on_time() {
    ...
    let options = MqttOptions::new("dummy", "mqtt://127.0.0.1:1880").unwrap();
...
async fn idle_connection_triggers_pings_on_time() {
    ...
    let mut options = MqttOptions::new("dummy", "mqtt://127.0.0.1:1885").unwrap();
...
async fn some_outgoing_and_no_incoming_should_trigger_pings_on_time() {
    let keep_alive = 5;
    let mut options = MqttOptions::new("dummy", "mqtt://127.0.0.1:1886").unwrap();
...
async fn some_incoming_and_no_outgoing_should_trigger_pings_on_time() {
    let keep_alive = 5;
    let mut options = MqttOptions::new("dummy", "mqtt://127.0.0.1:2000").unwrap();
...
async fn detects_halfopen_connections_in_the_second_ping_request() {
    let mut options = MqttOptions::new("dummy", "mqtt://127.0.0.1:2001").unwrap();
...
async fn requests_are_blocked_after_max_inflight_queue_size() {
    let mut options = MqttOptions::new("dummy", "mqtt://127.0.0.1:1887").unwrap();
...
async fn requests_are_recovered_after_inflight_queue_size_falls_below_max() {
    let mut options = MqttOptions::new("dummy", "mqtt://127.0.0.1:1888").unwrap();
...
async fn packet_id_collisions_are_detected_and_flow_control_is_applied() {
    let mut options = MqttOptions::new("dummy", "mqtt://127.0.0.1:1891").unwrap();
...
async fn next_poll_after_connect_failure_reconnects() {
    let options = MqttOptions::new("dummy", "mqtt://127.0.0.1:3000").unwrap();
...
async fn reconnection_resumes_from_the_previous_state() {
    let mut options = MqttOptions::new("dummy", "mqtt://127.0.0.1:3001").unwrap();
    options.set_keep_alive(Duration::from_secs(5));
...
async fn reconnection_resends_unacked_packets_from_the_previous_connection_first() {
    let mut options = MqttOptions::new("dummy", "mqtt://127.0.0.1:3002").unwrap();
    options.set_keep_alive(Duration::from_secs(5));
...
async fn state_is_being_cleaned_properly_and_pending_request_calculated_properly() {
    let mut options = MqttOptions::new("dummy", "mqtt://127.0.0.1:3004").unwrap();
    options.set_keep_alive(Duration::from_secs(5));
